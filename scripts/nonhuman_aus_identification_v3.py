...
def extract_upstream(seq, primer, qs, allele, len_dict):
    ...
    span_tuple_lst = [m.span() for m in re.finditer(r'G{%s,}'%nG, seq)]
    if span_tuple_lst != []:
        gs, ge = span_tuple_lst[0]
        if span_tuple_lst[0][0] <= nLen:
            ATG_loci = []
            upstreamSeq = seq[ge:qs]
            for i in range(len(seq[ge:qs])-2):
                if seq[ge:qs][i:i+3] == "ATG":
                    ATG_loci.append(i)
            if ATG_loci == []:
                return ["", ""]
            else:
                leader_start = sorted(zip(ATG_loci, 
                                          [abs(len_dict[allele]-(len(seq[ge:qs])-x)) for x in ATG_loci]), 
                                      key=lambda x:x[1])[0][0]
                return (seq[ge:qs], leader_start)
