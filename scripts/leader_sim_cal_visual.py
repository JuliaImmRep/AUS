...
for chain in ["igh", "igk", "igl"]:
    ...
    df_seq["leader"] = df_seq.apply(lambda x: x.Sequence[x.leaderStart:], axis=1)
    gene_leaders_dict = df_seq.groupby("gene")["leader"].unique().to_dict()
    ...
    pd.DataFrame(iden_dict, index=gene_lst).to_csv("human.%s.leader.similarity.matrix.csv"%chain)


    df = pd.read_csv("human.%s.leader.similarity.matrix.csv"%chain, index_col=0)
    ...
    plt.savefig("human.%s.leader.similarity.matrix.jpg"%chain, dpi=50, bbox_inches="tight")
