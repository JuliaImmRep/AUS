'''
    This script is for heavy chain
'''
import pandas as pd
import csv, sys
...
def main():
    ...
    for i, species in enumerate(["rhesus", "fascicularis", "mouse", "rat"]):
        ...
        df_human = pd.read_csv("upstream_sequence_filter_20210324/human.igh.upstream.score.and.filter.q.0.50.comb.txt", sep="\t")
        df_other = pd.read_csv("upstream_sequence_filter_20210324/%s.igh.upstream.score.and.filter.q.0.50.comb.txt"%species, sep="\t")
        ...
        df_human = df_human.groupby("gene").leader.unique()
        ...
        df_other = df_other.groupby("gene").leader.unique()
        ...
        if i != 0:
            ...
            dF = df
    dF.to_csv("human_other_species_leader_similarity_core_and_noncore_igh.csv")

    df_data = pd.read_csv("human_other_species_leader_similarity_core_and_noncore_igh.csv", index_col=0)

    palette = ["blue", "red"]
    palette = ["navy", "darkorchid"]
    ...
    plt.savefig("leader_similarity_cmp_bwt_core_noncore_for_diff_species_igh.jpg", dpi=300, bbox_inches="tight") 
