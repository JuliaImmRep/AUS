...
def extract_upstream(seq, primer, qs, allele, len_dict):
    ...
    nLen = 5  # the maximum distance to the end of primer that are accepted as the valid starting base hit
    
    primerLen = len(primer)
    flag = 0  # flag indicate if a primer is found
    for s in range(len(seq)-primerLen+1):
        subseq = seq[s:s+primerLen]
        nmismatch = hmd4str(subseq, primer)
        if nmismatch <= nMismatch:
            flag = 1 
            break
    if flag == 0:  # deal with case in which primer is not found
        return ["", ""]
    else:  # deal with case in which primer is found
        span_tuple_lst = [m.span() for m in re.finditer(r'G{%s,}'%nG, seq[s+primerLen:])]
        if span_tuple_lst != []:
            gs, ge = span_tuple_lst[0]
            if span_tuple_lst[0][0] <= nLen:
                ATG_loci = []
                upstreamSeq = seq[s+primerLen+ge:qs]
                for i in range(len(seq[s+primerLen+ge:qs])-2):
                    if seq[s+primerLen+ge:qs][i:i+3] == "ATG":
                        ATG_loci.append(i)
                if ATG_loci == []:
                    return ["", ""]
                else:
                    leader_start = sorted(zip(ATG_loci, 
                                              [abs(len_dict[allele]-(len(seq[s+primerLen+ge:qs])-x)) for x in ATG_loci]), 
                                          key=lambda x:x[1])[0][0]
                    return (seq[s+primerLen+ge:qs], leader_start)
